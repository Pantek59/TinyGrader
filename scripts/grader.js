...
function loadMarks() {
    let file = document.getElementById("load_button").files[0];
...
function read_config() {
    ...
    reader.addEventListener('load', (event) => {
        ...
        $("#config_area").show();
        try {grader_configuration = TOML.parse(document.getElementById("config_area").value)}
        catch (e) {console.log(e)}
    });
    reader.readAsText(file, "utf-8");
...
function read_class() {
    ...
    const reader = new FileReader();
    reader.addEventListener('load', (event) => {
        let class_raw = sanitize(reader.result);
...
function sanitize(input) {
    input = replaceAll(input,"ä", "ae");
    input = replaceAll(input,"ö", "oe");
    input = replaceAll(input,"ü", "ue");
    input = replaceAll(input,"Ä", "Ae");
    input = replaceAll(input,"Ö", "Oe");
    input = replaceAll(input,"Ü", "Ue");
...
function init_grading(loadedMarks) {
    let active_group = [];
    ...
    if (loadedMarks === undefined) {
        ...
        // Load active students from marking file
        let keys = Object.keys(loadedMarks);
        let group = loadedMarks.config.class;
        schemeLabel = loadedMarks.config.scheme;

        for (let k of keys) {
            if (k !== "config") {
                let obj = [];
                obj.push(k.replace(/_/g, " "));
                obj.push(group);
    ...
    if (loadedMarks !== undefined){
        // Fill in loaded marks
        for (let student in loadedMarks){
            if (student !== "config") {
                for (let asp in loadedMarks[student]) {
                    for (let dim in loadedMarks[student][asp]) {
                        $('[data-student="' + replaceAll(student,"_", " ") + '"][data-dimension="' + dim + '"][data-aspect="' + asp + '"][data-type!="group_mark"]').each(function () {
                            $(this).val(loadedMarks[student][asp][dim]);
                            update_grading($(this).get(0));
                        })
                    }
                }
                // Load comment field
...
function update_grading(element) {
    ...
    // Check whether entry within scale
    if (element.value != "") {
        ...
        $("[data-dimension='grade'][data-student='" + affected_student + "'][data-aspect='" + affected_aspect + "']").val("");
    }
...
function calcTotalGrades() {
    $("[data-dimension='total_grade']").each(function () {
        ...
        if (counter < grade_fields.length) {
            $("[data-dimension='total_grade'][data-student='"+ affected_student +"']").val("");
        }
        else {
...
function saveClass() {
    ...
    let aspectList = [];
    let dimensionList = [];
    let name = prompt("Please enter name or identifier!", "Default Reviewer");
    if (name) {
